...
@section('content')
    ...
    <form action="{{route('useranime.update', $useranime)}}" method="post">
        ...
        <input type="text" name="status_id" placeholder="Status id" value="{{old('status_id') ?? $useranime->status_id}}"
        ...
        <input type="date" name="date_voted" placeholder="Date voted" value="{{old('date_voted') ?? $useranime->date_voted}}"
