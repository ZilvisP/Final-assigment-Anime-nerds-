...
@section('content')
    ...
    <form action="{{route('anime.update', $anime)}}" method="post">
        ...
        <input type="text" name="description" placeholder="Description" value="{{old('description') ?? $anime->description}}"
               class="@error('description')is-invalid @enderror"><br>
        <input type="text" name="release_date" placeholder="Release date" value="{{old('release_date') ?? $anime->release_date}}"
